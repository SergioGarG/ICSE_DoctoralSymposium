...
However, those applications are not developed through well-defined system development process.
This fact leads to several time consuming tasks as the necessity of starting projects from scratch, the inability of reusing already developed packages and of exchanging the already implemented ones, etc.
Besides, robot applications are increasingly based on \emph{teams} of autonomous robots that work collaboratively to accomplish a mission.
The missions that can be achieved by a team of are more complex than the ones that can be achieved by a single robot, but the complexity of the application is also increased.
